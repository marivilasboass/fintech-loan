...
import { Colors, Typography, View, TitleAndValueRow, Spacing, Message } from '~/newUI'
...
export default class ConfirmLoanC extends React.PureComponent<Props> {
  renderYearTaxes = (props) => {
    const { loan } = this.props
    return (
      <Typography.T2 {...props}>
        <Typography.T1 format='percentage'>{loan.interestPercentagePerMonth}</Typography.T1>
  ...
  renderDueDate = (props) => {
    ...
    return (
      <Typography.T2 {...props}>
        <Typography.T1>dia {loan.bestPayDay} </Typography.T1>
        de cada mês
      </Typography.T2>
    )
  }
  ...
  renderNumberOfInstallments = (props) => {
    const { loan } = this.props
    return (
      <Typography.T2 {...props}>
        <Typography.T1>{loan.numberOfInstallments} </Typography.T1>
        meses
      </Typography.T2>
    )
  }
    ...
    return (
      ...
      >
        <View style={styles.content}>
          <Summary>
            <Summary.Card style={styles.fistCard}>
              ...
              <TitleAndValueRow style={styles.row} title={{ text: 'Prazo' }} value={{ component: this.renderNumberOfInstallments }} />
        ...
        <Message variant='info' style={styles.message}>
          <Typography.T3>IOF de {format('currency', math.divide(loan.iofAmountCents, 100))}. Custo efetivo total
            de {format('percentage', loan.cetPercentagePerYear)} a.a. Valor total a pagar de {format('currency', math.divide(loan.totalAmountCents, 100))}.</Typography.T3>
        </Message>
      </ConfirmLoanBase>
    )
  }
